...
{
    [TestClass]
    public partial class PredictTest : ParserResultTest<Predict>
    {
        protected override Parser<Predict> MyParser
        {
            get
            {
                return SQLParserComponents.P_Predict;
            }
        }

        private Type Cmp = typeof(ComparePredict);
        private Type Conj = typeof(ConjunctionPredict);
        private Type Disj = typeof(DisjunctionPredict);
        ...
        private void CheckPredict(Predict p, string prompt)
        {
            ...
            if (T == typeof(ConjunctionPredict))
            {
                ConjunctionPredict conjP = p as ConjunctionPredict;
                Assert.AreEqual(prompt2 + GetIntElement(), prompt2 + conjP.PredictList.Count);
                for (int i = 0; i < conjP.PredictList.Count; i++)
                {
                    CheckPredict(conjP.PredictList[i], prompt + "Conj[" + i + "]->");
                }
            }
            else if (T == typeof(DisjunctionPredict))
            {
                DisjunctionPredict disjP = p as DisjunctionPredict;
                Assert.AreEqual(prompt2 + GetIntElement(), prompt2 + disjP.PredictList.Count);
                for (int i = 0; i < disjP.PredictList.Count; i++)
                {
                    CheckPredict(disjP.PredictList[i], prompt + "Disj[" + i + "]->");
                }
            }
        }
