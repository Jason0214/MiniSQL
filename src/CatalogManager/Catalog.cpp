...
void Catalog::UpdateDatabaseTableIndex(const string & db_name, uint32_t new_addr){
	RecordResult* result = this->FindDatabaseBlock(db_name);
	uint32_t* database_info = (uint32_t*)(result->block_ptr->GetDataPtr(result->index, 1));
	database_info[1] = new_addr;
	buffer_manager.ReleaseBlock((Block* &)result->block_ptr);
	delete result;
}

void Catalog::UpdateDatabaseIndexIndex(const string & db_name, uint32_t new_addr){
	RecordResult* result = this->FindDatabaseBlock(db_name);
	uint32_t* database_info = (uint32_t*)(result->block_ptr->GetDataPtr(result->index, 1));
	database_info[3] = new_addr;
	buffer_manager.ReleaseBlock((Block* &)result->block_ptr);
	delete result;
}
...
/* find the block where `table_name` should insert in */
	...
	// first record to insert
		index_manager.insertEntry(index_tree_root, BPTree, &ConstChar<32>(table_name.c_str()), this->table_data_addr);
...
/* do real insertion */
	...
	// if no primary key specified, create a secondary index
		this->CreateIndex("DEFAULT", table_name,  0);
	}
	else{
...
void Catalog::CreateIndex(const string & index_name, const string & table_name, int8_t secondary_key_index){
	if(!this->database_selected) throw DatabaseNotSelected();
/* do finding */
