...
void Catelog::CreateTable(const string & table_name, string* attr_name_list, DBenum* attr_type_list, int attr_num, int key_index){
	if (!this->database_selected) throw DatabaseNotSelected();
	// get index
	IndexManager<char*> index_manager;
...
void Catelog::GetTableAttr(TableMeta & table_meta){
	if(!this->database_selected) throw DatabaseNotSelected();
...
void Catelog::CreateIndex(const string & table_name, int8_t secondary_key_index){
	if(!this->database_selected) throw DatabaseNotSelected();	
...
uint32_t Catelog::GetIndex(const string & table_name, int8_t secondary_key_index){
	// get B+ tree
